...
# Querying the default tenant to validate authentication
data "vastdata_tenant" "default" {
  provider = vastdata.GCPCluster # Explicitly mapping the aliased provider
  name     = "default"
}
...
output "auth_test_status" {
  value       = "SUCCESS: Connection established to VAST VMS!"
  description = "Confirms if authentication succeeded"
}

output "tenant_name" {
  description = "The name of the retrieved tenant in the VAST cluster"
  value       = data.vastdata_tenant.default.name
}

output "tenant_id" {
  description = "The ID of the retrieved tenant in the VAST cluster"
